...
def login():
    ...
    connection = con.connect(host="localhost", user="root", password="root", database="irctc")
...
def Register():
    f = tk.PhotoImage(file="C:/Users/akash/Downloads/back.png")
...
# Photo goes here
    ...
    f=tk.PhotoImage(file="C:/Users/akash/Downloads/train.png")
    background_label=tk.Label(w1,image=f)
...
# Creating frame
    ...
    frame_input=tk.Frame(w1,bg='')
    frame_input.place(x=340,y=130,height=500,width=400)
...
#LABELLING AND PACKING
    ...
    x.place(x=400,y=150)
    user.place(x=420,y=230)
    password.place(x=420,y=340)
...
def booked():
    ...
    if number_of_passengers==2:
        import mysql.connector as my
        con=my.connect(host="localhost",user='root',password='root',db='irctc')
        cursor=con.cursor()
...
def train():
    ...
    con=connection.connect(host="localhost",user="root",password="root",db="irctc")
    cursor=con.cursor()
